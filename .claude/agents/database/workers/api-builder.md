...
- `Docs L1/L2 (@neuledge/context first; Context7 MCP fallback only for L1 miss/stale/insufficient)` - Use FIRST when implementing tRPC patterns or Supabase Auth
  - Trigger: Before writing any tRPC router, procedure, or Supabase Auth integration
  // L2 fallback only when @neuledge/context is missing/stale/insufficient: - Key tools: `mcp__context7__resolve-library-id` then `mcp__context7__get-library-docs` for tRPC 11.x and Supabase Auth patterns
...
- `mcp__supabase__*` - Use WHEN integrating with Supabase Auth services
  ...
  - Key tools:
    - `Context7 (Docs L1/L2 (@neuledge/context first; Context7 MCP fallback only for L1 miss/stale/insufficient)) - Supabase MCP unavailable in default config` for Auth documentation and JWT patterns
...
1. If Docs L1/L2 (@neuledge/context first; Context7 MCP fallback only for L1 miss/stale/insufficient) is unavailable: Proceed with tRPC 10.x patterns and warn about potential API differences
...
1. **Assess the API Task:**
   - IF implementing tRPC routers → Use Docs L1/L2 for tRPC 11.x patterns: @neuledge/context first; Context7 MCP fallback only for L1 miss/stale/insufficient
...
2. **Smart MCP Usage:**
   - When creating new tRPC routers, first use Docs L1/L2 for current tRPC createRouter patterns: @neuledge/context first; Context7 MCP fallback only for L1 miss/stale/insufficient
...
- Always use Docs L1/L2 for tRPC 11.x breaking changes before implementing routers: @neuledge/context first; Context7 MCP fallback only for L1 miss/stale/insufficient
...
5. **MCP Tools Used**: Which Docs L1/L2 (@neuledge/context first; Context7 MCP fallback only for L1 miss/stale/insufficient) or mcp**supabase** resources were consulted

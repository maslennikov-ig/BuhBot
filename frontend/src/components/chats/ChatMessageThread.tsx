...
export function ChatMessageThread({ chatId }: ChatMessageThreadProps) {
  ...
  // Track scroll position and load older messages at top
  ...
  React.useEffect(() => {
    if (scrollRef.current && messages.length > 0 && !initialScrollDoneRef.current) {
  ...
  // Smooth scroll to bottom (for "jump to latest" button)
    ...
    <div className="relative">
      ...
      {!isAtBottom && (
        <button
          ...
          className="absolute bottom-6 right-6 flex items-center gap-2 rounded-full bg-[var(--buh-accent)] px-4 py-2 text-sm font-medium text-white shadow-lg transition-all hover:bg-[var(--buh-accent-hover)]"
        >

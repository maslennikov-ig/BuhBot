...
_Target URL currently missing in sidebar, needs to be added or placed under Settings._
...
_Target URL currently missing in sidebar._
...
- **Actions:**
  ...
  - _Note:_ No "Create User" needed yet (users come from Auth provider), but "Invite" mock button is acceptable.
...
- **Fix `/sla` and `/reports`:** These are future features.
  - _Requirement:_ Create "Coming Soon" placeholder pages for these routes so links don't 404. Use the `GlassCard` component and a "Construction" illustration.
...
_After generating the spec, please prompt to run the planning command._

        ...
        {isOpen && (
          ...
          >
            ...
            <div className="p-4 border-b border-[var(--buh-border)] bg-[var(--buh-surface)]/50">
              <p className="text-sm font-medium text-[var(--buh-foreground)] truncate">{email}</p>
              <p className="text-xs text-[var(--buh-foreground-muted)] mt-0.5">Личный аккаунт</p>
            </div>
            ...
            <div className="p-2 flex flex-col gap-1">
              {menuItems.map((item) => (
                ...
                >
                  <item.icon
                    className={`w-4 h-4 ${item.primary ? 'text-[var(--buh-primary)]' : ''}`}
                  />
                  {item.label}
                </Link>
              ))}

...
// L2 fallback only when @neuledge/context is missing/stale/insufficient: 1. `mcp__context7__resolve-library-id` - Find "qdrant-js" or "jina-ai"
// L2 fallback only when @neuledge/context is missing/stale/insufficient: 2. `mcp__context7__get-library-docs` - Get specific topic docs
...
1. **For Collection Issues**:
   ...
   ```markdown
   Use Docs L1/L2: @neuledge/context first; Context7 MCP fallback only for L1 miss/stale/insufficientresolve-library-id: "qdrant-js"
   Then Docs L1/L2 (@neuledge/context first; Context7 MCP fallback only for L1 miss/stale/insufficient)get-library-docs with topic: "collections"
...
2. **For Query Issues**:
   ...
   ```markdown
   Use Docs L1/L2: @neuledge/context first; Context7 MCP fallback only for L1 miss/stale/insufficientresolve-library-id: "qdrant-js"
   Then Docs L1/L2 (@neuledge/context first; Context7 MCP fallback only for L1 miss/stale/insufficient)get-library-docs with topic: "filters"
   Validate: filter syntax, payload field matching
   ```

3. **For Embedding Issues**:
   ```markdown
   Use Docs L1/L2: @neuledge/context first; Context7 MCP fallback only for L1 miss/stale/insufficientresolve-library-id: "jina-ai"
   Then Docs L1/L2 (@neuledge/context first; Context7 MCP fallback only for L1 miss/stale/insufficient)get-library-docs with topic: "embeddings"
